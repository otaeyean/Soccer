...
class CreatePostPage extends StatefulWidget {
  final String selectedTeam;  // 선택된 팀을 전달받음

  CreatePostPage({required this.selectedTeam}); // 팀 정보를 받는 생성자
...
class _CreatePostPageState extends State<CreatePostPage> {
  ...
  void _createPost(BuildContext context) async {
    ...
    try {
      // 선택된 팀에 맞게 게시글 저장
      await FirebaseFirestore.instance.collection('board_posts').add({
        'title': titleController.text.trim(),
        'content': contentController.text.trim(),
        'userId': _user!.uid,
        'username': username,
        'team': widget.selectedTeam,  // 팀 정보 추가
        'timestamp': DateTime.now(),
        'commentCount': 0,
      });

      Navigator.pop(context); // 게시글 작성 후 이전 페이지로 돌아가기
    } catch (e) {
      print("게시글 작성 오류: $e");
      ScaffoldMessenger.of(context).showSnackBar(
